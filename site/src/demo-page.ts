...
function render() {
  ...
  summary.textContent = `${total} sensitive values replaced. Repeated values keep the same token.`;
}
function reset() { rawLog.value = sample; render(); }
byId<HTMLButtonElement>('scrub-button').addEventListener('click', render);
byId<HTMLButtonElement>('reset-demo').addEventListener('click', () => { reset(); rawLog.focus(); });
